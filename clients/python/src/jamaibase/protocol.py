...
PositiveNonZeroInt = Annotated[
    int, Field(gt=0, description="Positive non-zero integer.")
]
...
class RAGParams(BaseModel):
    table_id: str = Field(
        description="Knowledge Table ID", examples=["my-dataset"], min_length=2
    )
...
class VectorSearchRequest(RAGParams):
    ...
    search_query: str = Field(
        description="Query used to retrieve items from the KB database."
    )
    ...
    def validate_gen_config(self) -> Self:
        if self.gen_config is not None:
            ...
            if "embedding_model" in self.gen_config:
                self.gen_config = EmbedGenConfig.model_validate(
                    self.gen_config
                ).model_dump()
            else:
                self.gen_config = ChatRequest.model_validate(
                    self.gen_config
                ).model_dump()
        return self
    ...
    def check_gen_configs(self) -> Self:
        for i, col in enumerate(self.cols):
            ...
            if col.vlen > 0:
                ...
                if num_prompts > 2:
                    self.cols[i].gen_config["messages"] = self.cols[i].gen_config[
                        "messages"
                    ][:2]
                elif num_prompts == 2:
                    pass
...
class TableSchemaCreate(TableSchema):
    ...
    def check_cols(self) -> Self:
        if len(set(c.id.lower() for c in self.cols)) != len(self.cols):
            raise ValueError(
                "There are repeated column names (case-insensitive) in the schema."
            )
        if sum(c.id.lower() in ("id", "updated at") for c in self.cols) > 0:
            raise ValueError(
                "Schema cannot contain column names: 'ID' or 'Updated at'."
            )
...
class KnowledgeTableSchemaCreate(TableSchemaCreate):
    ...
    def check_cols(self) -> Self:
        super().check_cols()
        num_text_cols = sum(
            c.id.lower() in ("text", "title", "file id") for c in self.cols
        )
        if num_text_cols != 0:
            raise ValueError(
                "Schema cannot contain column names: 'Text', 'Title', 'File ID'."
            )
        return self
...
class EmbedFileRequest(BaseModel):
    ...
    chunk_size: Annotated[
        int,
        Field(
            description="Maximum chunk size (number of characters). Must be > 0.", gt=0
        ),
    ] = 1000
...
class FileUploadRequest(BaseModel):
    file_path: Annotated[
        str, Field(description="File path of the document to be uploaded.")
    ]
...
class TableDataImportRequest(BaseModel):
    ...
    stream: Annotated[
        bool, Field(description="Whether or not to stream the LLM generation.")
    ] = True
    ...
    delimiter: Annotated[
        str,
        Field(
            description='The delimiter of the file: can be "," or "\\t". Defaults to ",".'
        ),
    ] = ","
