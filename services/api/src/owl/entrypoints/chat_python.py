...
def assemble_script(body: ChatCompletionRequest):
    messages = [
        message.content
        if isinstance(message.content, str)
        else "\n".join(d["text"] for d in message if d["type"] == "text")
...
async def chat_completion(body: ChatCompletionRequest):
            ...
            for i, char in enumerate(output):
                chunk = ChatCompletionResponse(
                    ...
                    usage=CompletionUsage(
                        prompt_tokens=len(script),
                        completion_tokens=i + 1,
                        total_tokens=len(script) + i + 1,
                    ),
                )
    ...
    return ChatCompletionResponse(
        ...
        usage=CompletionUsage(
            ...
            total_tokens=len(script) + len(output),
        ),
    )
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "owl.entrypoints.chat_python:app",
        reload=False,
