<script lang="ts">
	...
	async function saveEditCell(
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/owl/gen_tables/action/rows`, {
			method: 'PATCH',
			headers: {
	...
	function keyboardNavigate(e: KeyboardEvent) {
		...
		// 	selectedRows = [
		// 		...selectedRows.filter((i) => !tableRowsState?.some(({ ID }) => ID === i)),
		// 		...tableRowsState.map(({ ID }) => ID)
		// 	];
		// }
	...
	>
		...
		>
			...
			>
				...
				<ColumnHeader tableType="action" {tableData} {refetchTable} {readonly} />
			</div>
			...
			{#if tableRowsState.rows && !tableRowsState.loading}
				...
				{#each tableRowsState.rows as row (row.ID)}
					<div
