<script lang="ts">
	...
	import { actionTableStaticCols } from '$lib/constants';
	...
	function handleColumnHeaderClick(column: GenTableCol) {
		...
		if (dblClickTimer) {
			...
			dblClickTimer = null;
			if (!readonly && !actionTableStaticCols.includes(column.id)) {
	...
	async function handleSaveColumnTitle(
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/action/columns/rename`, {
			method: 'POST',
			headers: {
		...
		if (response.ok) {
			...
			logger.error('ACTIONTBL_COLUMN_RENAME', responseBody);
	...
	async function handleSaveOrder() {
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/action/columns/reorder`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				'x-project-id': $page.params.project_id
	...
	async function saveEditCell(
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/action/rows/update`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				'x-project-id': $page.params.project_id
			...
			>
				...
				{#each tableData.cols as column, index (column.id)}
					...
					>
						...
						{#if (!actionTableStaticCols.includes(column.id) || colType === 'output') && !readonly}
							<ColumnDropdown
								tableType="action"
