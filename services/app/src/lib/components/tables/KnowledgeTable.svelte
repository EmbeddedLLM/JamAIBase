<script lang="ts">
	...
	async function saveEditCell(
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/owl/gen_tables/knowledge/rows`, {
			method: 'PATCH',
			headers: {
...
{#if tableData}
	...
	>
		<div
			...
			role="grid"
			style={tableRowsState.rows?.length !== 0
				? `grid-template-rows: 36px ${
						tableRowsState.rows && !tableRowsState.loading
							? `repeat(${tableRowsState.rows.length}, min-content)`
							: 'minmax(0, 1fr)'
					};`
				: undefined}
		...
		>
			...
			{#if tableRowsState.rows && !tableRowsState.loading}
				{#if tableRowsState.rows.length > 0}
					...
					{/each}
				{:else if tableRowsState.rows.length === 0}
					<div
						role="row"
						...
						class="sticky top-[40px] z-0 grid h-min place-items-start"
					>
						<div
							role="gridcell"
							class="sticky left-1/2 flex h-full w-max -translate-x-1/2 flex-col items-center justify-center p-1"
						>
							<NoRowsGraphic class="h-[16rem]" />
							<span class="mt-4 text-base sm:text-lg">Upload Document</span>
							<span class="text-xs text-[#999] sm:text-sm">
								Select a document to start generating your table
							</span>
							<Button onclick={handleUploadClick} class="pointer-events-auto mt-2">
								Browse Document
							</Button>
						</div>
					</div>
