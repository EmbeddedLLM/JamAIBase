<script lang="ts">
	...
	let {
		organizationData,
		isDeletingExtKey = $bindable()
	}: {
		organizationData: OrganizationReadRes | undefined;
		isDeletingExtKey: { open: boolean; value: string | null };
	} = $props();

	let loadingDeleteExtKey = $state(false);
</script>

<Dialog.Root
	bind:open={() => isDeletingExtKey.open,
	(v) => (isDeletingExtKey = { ...isDeletingExtKey, open: v })}
>
	<Dialog.Content
		data-testid="delete-ext-key-dialog"
	...
	>
		...
		<div class="flex flex-col items-start gap-2 p-8">
			...
			<p class="text-sm text-text/60">
				...
				<span class="font-medium text-black [word-break:break-word] data-dark:text-white">
					`{PROVIDERS[isDeletingExtKey.value ?? ''] || isDeletingExtKey.value}`
		...
		<Dialog.Actions class="bg-[#f6f6f6] data-dark:bg-[#303338]">
				...
				use:enhance={({ formData, cancel }) => {
					...
					return async ({ update, result }) => {
						if (result.type === 'failure') {
							...
							toast.error('Error deleting external key', {
							...
							isDeletingExtKey = { ...isDeletingExtKey, open: false };
						}

						loadingDeleteExtKey = false;
						update({ reset: false });
					};
				}}
			...
			>
				<input type="hidden" name="provider" value={isDeletingExtKey.value} />
				...
				<Button
					type="submit"
					variant="destructive"
					loading={loadingDeleteExtKey}
					disabled={loadingDeleteExtKey}
					class="grow px-6"
				>
					Delete
				</Button>
