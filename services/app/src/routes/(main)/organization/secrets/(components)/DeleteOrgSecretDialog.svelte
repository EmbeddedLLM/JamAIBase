<script lang="ts">
	...
	let {
		isDeletingOrgSecret = $bindable()
	}: {
		isDeletingOrgSecret: { open: boolean; value: string | null };
	} = $props();

	let loadingDeleteOrgSecret = $state(false);
</script>

<Dialog.Root
	bind:open={
		() => isDeletingOrgSecret.open,
		(v) => (isDeletingOrgSecret = { ...isDeletingOrgSecret, open: v })
	}
>
	<Dialog.Content
		data-testid="delete-org-secret-dialog"
	...
	>
		...
		<Dialog.Actions class="bg-[#f6f6f6] data-dark:bg-[#303338]">
			<form
				...
				id="deleteOrgSecret"
				use:enhance={() => {
					loadingDeleteOrgSecret = true;

					return async ({ update, result }) => {
						if (result.type === 'failure') {
							const data = result.data as any;
							toast.error('Error deleting organization secret', {
							...
							isDeletingOrgSecret = { ...isDeletingOrgSecret, open: false };
						}

						loadingDeleteOrgSecret = false;
						update({ reset: false });
					};
				}}
				action="?/delete-org-secret"
				class="flex gap-2 overflow-x-auto overflow-y-hidden"
			>
				<input type="hidden" name="name" value={isDeletingOrgSecret.value} />
				...
				<Button
					type="submit"
					variant="destructive"
					loading={loadingDeleteOrgSecret}
					disabled={loadingDeleteOrgSecret}
					class="grow px-6"
				>
					Delete
				</Button>
