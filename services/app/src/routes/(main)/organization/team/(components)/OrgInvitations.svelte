<script lang="ts">
	...
	import { RevokeOrgInviteDialog } from '.';
	...
	let orgInvites: VerificationCodeRead[] = $state([]);
	...
	async function getOrganizationInvites() {
		...
		try {
			...
			const response = await fetch(
				`${PUBLIC_JAMAI_URL}/api/owl/organizations/invites/list?${searchParams}`,
				{
			...
			if (response.status == 200) {
				...
				if (moreProjects.items.length) {
					orgInvites = [...orgInvites, ...moreProjects.items];
				} else {
				...
				toast.error('Failed to fetch organization invites', {
	...
	export async function refetchOrgInvites() {
	...
	<div class="flex items-center justify-between">
		<div class="flex w-min items-center gap-2">
			...
			<SorterSelect
				bind:sortOptions
				{sortableFields}
				refetchTables={refetchOrgInvites}
			...
			<Table.Body class="overscroll-y-auto">
				...
				{:else if !loadingInvitesError}
					{#if orgInvites.length > 0}
						{#each orgInvites as orgInvite}
							<Table.Row>
								<Table.Cell>
									<div class="flex items-center gap-2">
										<p title={orgInvite.user_email} class="line-clamp-1 break-all text-[#A62050]">
											{orgInvite.user_email}
										</p>
									</div>
									<!-- <span
											title={orgInvite.user.email}
											class="line-clamp-1 break-all text-[#98A2B3]"
										>
											{orgInvite.user.email}
										</span> -->
								</Table.Cell>
								<Table.Cell>
									{#if orgInvite.role}
										<span
											style:background={`${ROLE_COLORS[orgInvite.role]}32`}
										...
										>
											<span
												style:color={`${ROLE_COLORS[orgInvite.role]}`}
								...
								<Table.Cell>
									{new Date(orgInvite.created_at).toLocaleString(undefined, {
										month: 'short',
										day: 'numeric',
										year: 'numeric',
										hour: '2-digit',
								...
								<Table.Cell>
									{new Date(orgInvite.updated_at).toLocaleString(undefined, {
										month: 'short',
										day: 'numeric',
										year: 'numeric',
										hour: '2-digit',
								...
								<Table.Cell>
									{#if orgInvite.used_at}
										{new Date(orgInvite.used_at).toLocaleString(undefined, {
											month: 'short',
											day: 'numeric',
											year: 'numeric',
											hour: '2-digit',
								...
								<Table.Cell>
									{#if orgInvite.revoked_at}
										{new Date(orgInvite.revoked_at).toLocaleString(undefined, {
											month: 'short',
											day: 'numeric',
											year: 'numeric',
											hour: '2-digit',
								...
								<Table.Cell>
									{#if orgInvite.expiry}
										{new Date(orgInvite.expiry).toLocaleString(undefined, {
											month: 'short',
											day: 'numeric',
											year: 'numeric',
											hour: '2-digit',
								...
								<Table.Cell>
									<div class="flex w-min items-center gap-1.5 rounded-lg bg-[#F2F4F7] px-2 py-1.5">
										...
										<Button
											...
											onclick={() => {
												navigator.clipboard.writeText(orgInvite.id);
								...
								<Table.Cell class="min-w-[165px] max-w-[165px]">
									<Button
										...
										disabled={!!orgInvite.used_at || !!orgInvite.revoked_at}
										onclick={() => (revokingInvite = { open: true, value: orgInvite })}
										title="Revoke invite"
										class="h-7"
									>
										Revoke
						...
						<Table.Row>
							<Table.Cell colspan={999} class="pointer-events-none relative h-64 w-full">
								<div class="absolute left-1/2 flex -translate-x-1/2 flex-col">
									<span class="text-lg font-medium">No organization invites found</span>
								</div>
							</Table.Cell>
						</Table.Row>
					{/if}
					...
					<Table.Row>
						<Table.Cell colspan={999} class="pointer-events-none relative h-64 w-full">
							<div class="absolute left-1/2 flex w-[26rem] -translate-x-1/2 flex-col text-center">
								<span class="text-lg font-medium"> Error fetching organization invites </span>
								<span class="text-sm">
...
<RevokeOrgInviteDialog {refetchOrgInvites} bind:revokingInvite />
