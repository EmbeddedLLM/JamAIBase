...
<Dialog.Root
	bind:open={
		() => isAddingColumn.showDialog, (v) => (isAddingColumn = { ...isAddingColumn, showDialog: v })
	}
>
	<Dialog.Content
		...
		>
			...
			<div data-testid="datatype-select-btn" class="w-full space-y-1 px-4 sm:px-6">
				...
				<Select.Root type="single" bind:value={selectedDatatype}>
					...
					>
						{#each Object.keys(genTableDTypes).filter((dtype) => (isAddingColumn.type === 'output' || !dtype.endsWith('_code')) && (isAddingColumn.type === 'input' || dtype.startsWith('str') || dtype === 'file_code' || dtype === 'image')) as dType}
							<Select.Item
								value={dType}
			...
			{#if isAddingColumn.type == 'output'}
				{#if !selectedDatatype.endsWith('_code')}
					{@const isImageOutput = selectedDatatype === 'image'}
					<div class="space-y-1 px-4 sm:px-6">
						<Label class="text-xs sm:text-sm">Models</Label>

						<ModelSelect
							capabilityFilter={isImageOutput ? 'image_out' : 'chat'}
							bind:selectedModel
							selectCb={(model) => {
								if (!isImageOutput) {
