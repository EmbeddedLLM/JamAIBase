<script lang="ts">
	...
	async function handleAddConversation() {
		...
		if (!tableIDPattern.test(conversationName))
			return toast.error(
				'Conversation ID must contain only alphanumeric characters and underscores/hyphens/periods, and start and end with alphanumeric characters'
			);
