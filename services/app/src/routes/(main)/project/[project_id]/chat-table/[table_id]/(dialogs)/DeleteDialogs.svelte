<script lang="ts">
	...
	//TODO: Differentiate between pastChatAgents and pastChatConversations
	async function handleDeleteTable() {
		if (isLoading || !isDeletingTable) return;
		isLoading = true;

		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/chat/${isDeletingTable}`, {
			method: 'DELETE'
		});
		if (!response.ok) {
			const responseBody = await response.json();
			logger.error('CHATTBL_TBL_DEL', responseBody);
	...
	async function handleDeleteColumn() {
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/chat/columns/drop`, {
			method: 'POST',
			headers: {
		...
		if (!response.ok) {
			const responseBody = await response.json();
			logger.error('CHATTBL_COLUMN_DEL', responseBody);
	...
	async function handleDeleteRow() {
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/chat/rows/delete`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
		...
		if (!response.ok) {
			const responseBody = await response.json();
			logger.error('CHATTBL_ROW_DEL', responseBody);
