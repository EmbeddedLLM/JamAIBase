	...
	function refetchTable() {
		...
		if (Object.keys(streamingRows).length === 0) {
			if (searchQuery) {
				...
				invalidate('chat-table:slug').then(() => (isLoadingSearch = false));
			}
		}
	}
	...
	>
		...
		>
			<div class="flex items-center gap-2">
				...
				<ChatTableIcon class="flex-[0_0_auto] h-6 w-6 text-text" />
				<span
				...
				{#if tableData?.parent_id}
					<span
						...
						class="px-1.5 py-1 w-min whitespace-nowrap rounded-[0.1875rem] select-none capitalize font-medium text-xs line-clamp-1 break-all"
					>
						{tableData?.parent_id}
					</span>
				{/if}
			...
			{#if tableData && $genTableRows}
				...
				>
					<InputText
						...
						type="search"
						placeholder="Search"
						class="pl-8 h-9 [:has(>&)]:w-[unset] placeholder:not-italic bg-white border-[#E5E5E5] data-dark:border-[#666] rounded-full"
					>
		...
		<ColumnSettings
			bind:isColumnSettingsOpen
			showPromptTab={isColumnSettingsOpen.column?.id != 'AI'}
			tableType="chat"
		/>
	</section>
</div>
...
<DeleteDialogs bind:isDeletingColumn bind:isDeletingRow tableType="chat" />
