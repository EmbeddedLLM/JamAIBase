<script lang="ts">
	...
	import ChatTables from './ChatTables.svelte';
	import { AddColumnDialog, AddConversationDialog, AddRowDialog, DeleteDialogs } from './(dialogs)';
	...
	async function handleSaveOrder() {
		...
		const response = await fetch(`/api/v1/gen_tables/chat/columns/reorder`, {
			method: 'POST',
	...
	async function handleSaveEdit(
		...
		const response = await fetch(`/api/v1/gen_tables/chat/rows/update`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({
				table_id: tableData.id,
	...
	async function addRowFunction(data: any) {
		...
		if (response.status != 200) {
			const responseBody = await response.json();
			logger.error('CHATTBL_ROW_ADD', responseBody);
					...
					if (value.endsWith('\n\n')) {
						...
						for (const line of lines) {
							...
							try {
								...
								console.error('Error parsing:', sumValue);
								logger.error('CHATTBL_ROW_ADDSTREAMPARSE', { parsing: sumValue, error: err });
								continue;
							}
...
>
								...
								>
									{#if editMode}
										...
										<textarea
											...
											class="min-h-[150px] h-full w-full px-5 py-3 bg-transparent outline outline-setext-secondary data-dark:outline-[#5b7ee5] resize-none"
										/>
	...
	>
		...
		<ChatTables bind:isDeletingTable />
	</section>
</div>
