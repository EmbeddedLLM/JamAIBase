<script lang="ts">
	...
	async function handleDeleteTable() {
		...
		const response = await fetch(
			`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/knowledge/${isDeletingTable}`,
			{
				method: 'DELETE'
			}
		);
		if (!response.ok) {
			...
			logger.error('KNOWTBL_TBL_DEL', responseBody);
			alert('Failed to delete table: ' + (responseBody.message || JSON.stringify(responseBody)));
		} else {
			$pastKnowledgeTables = $pastKnowledgeTables.filter((table) => table.id !== isDeletingTable);
	...
	async function handleDeleteColumn() {
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/knowledge/columns/drop`, {
			method: 'POST',
			headers: {
		...
		if (!response.ok) {
			const responseBody = await response.json();
			logger.error('KNOWTBL_COLUMN_DEL', responseBody);
	...
	async function handleDeleteRow() {
		...
		const response = await fetch(`${PUBLIC_JAMAI_URL}/api/v1/gen_tables/knowledge/rows/delete`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
		...
		if (!response.ok) {
			const responseBody = await response.json();
			logger.error('KNOWTBL_ROW_DEL', responseBody);
