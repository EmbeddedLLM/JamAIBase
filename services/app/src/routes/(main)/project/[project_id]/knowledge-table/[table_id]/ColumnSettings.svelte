<script lang="ts">
	...
	//* User generated column settings
	...
	function resetValues() {
		if (!isColumnSettingsOpen.column?.id.includes('Embed')) {
			selectedTab = 'prompt';
		} else {
	...
	async function saveColumnTitle(
		...
		if (e.key === 'Enter') {
			const response = await fetch(`/api/v1/gen_tables/knowledge/columns/rename`, {
				method: 'POST',
				headers: {
	...
	async function saveColumnSettings() {
		...
		const response = await fetch(`/api/v1/gen_tables/knowledge/gen_config/update`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
			body: JSON.stringify({
				table_id: $page.params.table_id,
				column_map: {
					[isColumnSettingsOpen.column.id]: isColumnSettingsOpen.column.id.includes('Embed')
						? {
								embedding_model: selectedEmbedModel,
		...
		if (response.ok) {
			...
			const responseBody = await response.json();
			logger.error('KNOWTBL_COLUMN_UPDATESETTINGS', responseBody);
			alert(
...
>
	<div class="relative flex flex-col h-full w-full">
		...
		{#if isColumnSettingsOpen.column?.gen_config}
			...
			{:else if isColumnSettingsOpen.column.id.includes('Embed')}
				<div style="grid-template-rows: minmax(0, 1fr) 65px;" class="grow grid min-h-0">
					<div class="grid grid-rows-[min-content_1fr] px-6 py-5 overflow-auto">
