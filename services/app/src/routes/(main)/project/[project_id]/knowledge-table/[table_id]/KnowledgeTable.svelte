...
{#if tableData && $genTableRows}
	<div inert={isColumnSettingsOpen.showMenu} class="grow flex flex-col w-full min-h-0">
		...
		>
							...
							<DropdownMenu.Root>
								<DropdownMenu.Trigger asChild let:builder>
									<Button
										...
										class="flex-[0_0_auto] p-0 h-7 w-7 aspect-square rounded-full"
									>
			...
			{:else}
				<div
					class="fixed z-0 top-1/2 left-[45%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center justify-center gap-2 pointer-events-none"
				>
