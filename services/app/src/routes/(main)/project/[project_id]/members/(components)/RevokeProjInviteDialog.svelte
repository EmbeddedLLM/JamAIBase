<script lang="ts">
	...
	let {
		revokingInvite = $bindable(),
		refetchProjectInvites
	}: {
		revokingInvite: { open: boolean; value: VerificationCodeRead | null };
		refetchProjectInvites: () => void;
	} = $props();
...
<Dialog.Root
	bind:open={
		...
		(v) => {
			if (!v) {
				revokingInvite = { ...revokingInvite, open: false };
			}
		}
	}
>
	<Dialog.Content class="w-[clamp(0px,26rem,100%)] bg-white data-dark:bg-[#42464e]">
		...
		<div class="flex flex-col items-start gap-2 p-8 pb-10">
			...
			<p class="text-sm text-text/60">
				Do you really want to revoke invite
				<span class="font-medium text-black [word-break:break-word] data-dark:text-white">
					`{revokingInvite.value?.name || revokingInvite.value?.user_email}`
				</span>?
			</p>
		</div>
