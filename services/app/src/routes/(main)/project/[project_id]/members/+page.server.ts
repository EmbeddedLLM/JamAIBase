...
const { ORIGIN, OWL_URL, OWL_SERVICE_KEY /* RESEND_API_KEY */ } = env;
...
export const actions = {
	invite: async ({ cookies, fetch, locals, request }) => {
		...
		const addUserBody = await addUserRes.json();

		if (!addUserRes.ok) {
			if (![403].includes(addUserRes.status)) {
				logger.error('PROJTEAM_ADD_USER', addUserBody);
			}
			return fail(
				addUserRes.status,
				new APIError('Failed to add User', addUserBody as any).getSerializable()
			);
		}
		...
		/* const getInviteToken = await fetch(
			`${OWL_URL}/api/v2/projects/invites?${new URLSearchParams({
				user_email,
				project_id,
				role: user_role,
				valid_days
		...
		if (!getInviteToken.ok) {
			if (![403].includes(getInviteToken.status)) {
				logger.error('PROJTEAM_INVITE_TOKEN', inviteToken);
			}
			return fail(
		...
		if (RESEND_API_KEY) {
			const sendEmailRes = await fetch('https://api.resend.com/emails', {
				...
				body: JSON.stringify({
					...
					to: user_email,
					subject: 'You have been invited to join a project on JamAI Base',
					html: getInviteEmailBody(locals.user.email, inviteToken.id)
				})
			});

			if (!sendEmailRes.ok) {
				logger.error('PROJTEAM_INVITE_EMAIL', await sendEmailRes.json());
	...
	update: async ({ fetch, locals, request }) => {
		...
		const updateRoleRes = await fetch(
			`${OWL_URL}/api/v2/projects/members/role?${new URLSearchParams([
				['user_id', user_id],
	...
	remove: async ({ cookies, fetch, locals, request }) => {
		...
		if (!leaveOrgRes.ok) {
			logger.error('PROJTEAM_REMOVE_REMOVE', leaveOrgBody);
			return fail(
				leaveOrgRes.status,
				...
				<tr>
					...
					>
						<div class="main">
							...
							<h1>${inviterEmail} has invited you to join their project on JamAI Base</h1>

							<p>You have been invited to join a project on JamAI Base. Click the link below to accept the invitation:</p>

							<p><a href="${ORIGIN}/join-project?token=${inviteToken}">Join JamAI Base</a></p>
